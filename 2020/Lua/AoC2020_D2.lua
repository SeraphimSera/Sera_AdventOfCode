local passwords = [[4-7 z: zzzfzlzzz
3-4 l: blllk
8-11 j: jjjjjjjgjjjj
6-7 l: lllhlbl
...
-- Step 0.5: Why does lua not have a split function???
-- I borrowed (and tried to build) one
...
local function split(string, pattern)
    ...
    for str in string:gmatch("([^"..pattern.."]+)") do
        table.insert(_, str:match("^%s*(.-)%s*$"))
    end

    return _
end

-- Step 1: Split each line by newline + colon
local pw_table = {}
for i, line in ipairs(split(passwords, "\n")) do
    pw_table[i] = split(line, ":")
end

-- Step 2: Iterate over every password and check each one individually. Oh god. 
local valid_1 = 0
local valid_2 = 0

for i, pw_pair in pairs(pw_table) do
    -- Part 1
    local policy, pw = pw_pair[1], pw_pair[2]

    -- (Damn you and your tables, Lua.)
    _ = split(policy)
    local range, letter = _[1], _[2]

    range = split(range, "-")
    for i, num in ipairs(range) do
        range[i] = tonumber(num)
    end
    ...
    local count = 0
    for _ in pw:gmatch(letter) do count = count + 1 end


    if (left <= count) and (count <= right) then valid_1 = valid_1 + 1 end

    -- Part 2
    -- Make pw an iterable
    _ = {}
    for c in pw:gmatch(".") do table.insert(_, c) end   
    pw = _

    if (letter == pw[left] or letter == pw[right]) and (pw[left] ~= pw[right]) then
        valid_2 = valid_2 + 1
    end
end

print("There are "..valid_1.." valid passwords in part 1's system.")
print("There are "..valid_2.." valid passwords in part 2's system.")
