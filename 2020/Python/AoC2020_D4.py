...
order = ["byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid", "cid"]
req = order.copy()[:-1]
valid_1 = valid_2 = 0

for passport in batch.split("\n\n"):
    # No heck you, I'm not undoing this
    # It's beautiful

    # Also like, I didn't even need to do this. This is purely for practicing key sorts
    details = dict(sorted({k: v for k, v in [detail.split(":") for detail in passport.split()]}.items(), 
                    key=lambda x: order.index(x[0])))

    # Checking part 1 + 2
    passport = {k: v for k, v in zip(req, [details.get(key) for key in req])}
    if not None in passport.values():
        try:
            # This is only a dict for labelling purposes.
            reqs = {
            "byr": 2002 >= int(passport["byr"]) >= 1920, 
            "iyr": 2020 >= int(passport["iyr"]) >= 2010, 
            "eyr": 2030 >= int(passport["eyr"]) >= 2020, 

            "hgt": passport["hgt"][-2:] in ("cm", "in") and {
                "cm": 193 >= int(passport["hgt"][:-2]) >= 150,
                "in": 76 >= int(passport["hgt"][:-2]) >= 59 
            }[passport["hgt"][-2:]], 

            "hcl": passport["hcl"][0] == "#" and 
            int(passport["hcl"][1:], 16) and 
            len(passport["hcl"]) == 7,  
...
print(f"There are {valid_1} valid passports in part 1.")
print(f"There are {valid_2} valid passports in part 2.")