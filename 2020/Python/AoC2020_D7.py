_input = """dark olive bags contain 2 muted brown bags, 1 mirrored tomato bag, 4 bright black bags.
...
for rule in _input.splitlines():
    bag, contain = rule.split(" contain ")
    bag = bag.rsplit(maxsplit=1)[0]

    if "no" in contain:
        contain = 0
    else:
        contain = [item.rsplit(maxsplit=1)[0] for item in contain.split(", ")]
        contain = {k: int(v) for v, k in [item.split(maxsplit=1) for item in contain]}
    
    rules[bag] = contain

# Part 1
# This only took me two hours to do haha yes
bags = ["shiny gold", ]

for bag_check in bags:
    for bag, item in rules.items():
        if isinstance(item, dict) and bag_check in item:
            bags.append(bag)

del bags[0]
count_1 = len(set(bags))

# Part 2
# I gave up trying to do loops
...
def unnester(_dict):
    ...
    for k, v in _dict.items():
        if isinstance(rules[k], dict):
            num *= sum(unnester(rules[k]))
        else:
            num *= v
    return num

count_2 = unnester(rules["shiny gold"])

print(f"There are {count_1} bags that can hold a shiny gold bag.")
print(f"There are {count_2} bags that can be held in a shiny gold bag.")

# bag a has 2 c bags and 1 b bag.
# bag b has no bags.
# bag c has 3 bags. (which all contain nothing)
# bag a has 2 + (2 * 3) + 1 + (1 * 0) = 9 bags
